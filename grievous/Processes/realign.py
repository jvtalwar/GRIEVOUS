...
def _Create_Dictionary(newDictPath, dictStorage = "parquet"):
    # assert not os.path.isdir(newDictPath), f"Dictionary already exists at {newDictPath}" #<--- collisions might occur in parallel, change to if, else and exist_ok = True below
    if os.path.isdir(newDictPath):
        return None
    ...
    for chromosome in chromosomes:
        # Record aligned files. Keeping at chromosome level prevents record overwrite issues/collisions when parallelized 
        with open(os.path.join(newDictPath, f"Records/CHR_{chromosome}.aligned.record.txt"), "a") as alignedRecord: 
            pass

        #all the files which were used to update the dictionary. If --shutoff_dict_update is never used then is equivalent to aligned
...
def _Check_Dictionary(dbAlias):
    ...
    if dbAlias not in grievousDictAliases:
        if dbAlias is None: #Handle automatically when only one dictionary standard exists.
            ...
            Provide a current database/dictionary alias or provide a new one to create new empty database. Database aliases can be found with grievous list_dbs."""
...
Input(s): 1) file: String corresponding to the path of the file which the user is attempting to realign.
          ...
          5) file_type: String in {'pvar', 'ssf'} corresponding to the desire genomic file type to be realigned.
...
def Realign(file, dictionary, write_path, mapping, file_type, return_all, comment_characters, shutoff_dict_update):
    ...
    #Identify/Make necessary dictionary directory and load chromosome-level dictionary
    ...
    try:
        ...
        logger.info("parquet chromosome dictionary not found... Trying tsv")
        assert f"CHR_{gFile.Chrom}.tsv" in os.listdir(grievousDictionaryPath), f"No valid dictionary file found in dictionary {dictionary} for chromosome {gFile.Chrom}. Exiting..."
    ...
    if write_path is None:
        ...
        if (os.path.exists(os.path.join(write_path, "GRIEVOUS_Formatted", f"FormattedAndAlignedPvar_CHR{gFile.Chrom}.tsv"))) or (os.path.exists(os.path.join(write_path, "GRIEVOUS_Formatted", f"FormattedAndAlignedSSF_CHR{gFile.Chrom}.tsv"))):
            logger.warning(f"Previous GRIEVOUS Formatted file detected in write_path {write_path}. If this directory was used for a previous realignment, current files will be overwritten.") 
